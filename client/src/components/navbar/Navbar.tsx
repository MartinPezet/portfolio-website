...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    setLogoClass(currentFont.toLowerCase().replace(/ /g, "-"));
  }, [currentFont]);
    ...
    <header className={`header sticky z-10 flex justify-center${hasScrolled ? " top-2" : " top-4"}`}>
      <nav className="navbar relative flex flex-row items-center justify-between w-full gap-4 sm:gap-12 mx-4 mb-4 max-w-7xl 3xl:max-w-8xl py-4 px-4 sm:px-8 rounded-xl bg-header" id="navbar">
        <div className="flex items-center w-14 md:w-24">
          <p className={`cursor-pointer title-font logo default-font ${logoClass} ${hasScrolled ? "default-font-scrolled " + logoClass + "-scrolled" : ""}`} onClick={() => onInternalNavigationClick('home')}>M</p>
        </div>
